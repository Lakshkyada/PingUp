...
const CreatePost = () => {
  ...
  const handleSubmit = async () => {
        ...
        setLoading(true)
        const postType = images.length && content ? 'text_with_image' : image.length ?
        'image' : 'text'
        try {
                  ...
                  <div className='flex flex-wrap gap-2 mt-4'>
                    {
                       images.map((image, i)=>(
                          <div key={i} className='relative group'>
                              ...
                              <div onClick={()=>setImages(images.filter((__dirname,index)=>index !== i))} className='absolute hidden group-hover:flex justify-center items-center top-0 right-0 bottom-0 left-0 bg-black/40 rounded-md cursor-pointer'>
