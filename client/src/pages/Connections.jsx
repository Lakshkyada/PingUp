          ...
          <div className='inline-flex flex-wrap items-center border border-gray-200 rounded-md p-1 bg-white shadow-sm'>
            {
              dataArray.map((tab)=>(
                <div key={tab.label}>
                <button style={{height:25}} onClick={()=>setCurrentTab(tab.label)} key={tab.label} className={`flex items-center px-3 py-1 text-sm rounded-md transition-colors ${currentTab === tab.label ? 'bg-white font-medium text-black'
                ...
                dataArray.find((item)=>item.label===currentTab).value.map((user)=>(
                     <div key={user._id} className='w-full max-w-88 flex gap-5 p-6 bg-white shadow rounded-md '>
                        ...
                        <div className='flex-1'>
                            ...
                            <p className='text-sm text-gray-600'>{user.bio?.slice(0, 30) || "No bio"}...</p>
                            <div className='flex max-sm:flex-col gap-2 mt-4'>
                               ...
                               {
                                  currentTab === 'Pending' && user.requestType !== 'outgoing' && (
                               ...
                               {
                                  currentTab === 'Pending' && user.requestType === 'outgoing' && (
                                    <button disabled className='w-full p-2 text-sm rounded bg-slate-100 text-slate-500 cursor-not-allowed'>
                                        Requested
                                    </button>
                                  )
                               }
                               {
                                  currentTab === 'Connections' && (
                                    <button onClick={()=>navigate(`/messages/${user._id}`)} className=' flex w-full p-2 text-sm rounded bg-slate-100 hover:bg-slate-200 text-black active:scale-95 transition cursor-pointer'>
