...
import {Calendar, Verified, MapPin, PenBox} from 'lucide-react'
          ...
          <div className='w-full pt-16 md:pt-0 md:pl-36'>
              <div className='flex flex-col md:flex-row items-start justify-between'>
                   <div>
                      ...
                      <p className=''>{user.username ? `@${user.username}` : 'Add a username'}</p>
                   </div>
                   {/* Edit */}
                   {
                      !profileId && 
              ...
              <div className='flex flex-wrap items-center gap-x-6 gap-y-2 text-sm text-gray-500 mt-4'>
                  <span className='flex items-center gap-1.5'>
                   ...
                   <span className='flex items-center gap-1.5'>
                     ...
                     Joined <span className=''>{moment(user.createdAt).fromNow()}</span>
                  </span>
              </div>
              
               <div className='flex items-center gap-6 mt-6 border-t border-gray-200 pt-4'>
                   <div>
                      <span className='sm:text-xl font-bold text-gray-900'>{posts ? posts.length :0}</span>
