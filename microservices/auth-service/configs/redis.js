...
export const safeRedisGet = async (key) => {
  try {
    ...
    console.error(`Auth Service: Redis GET error for ${key}:`, err.message);
    return null;
  }
};
...
export const safeRedisSet = async (key, value, options = {}) => {
  try {
    ...
    console.error(`Auth Service: Redis SET error for ${key}:`, err.message);
    return false;
  }
};
...
export const safeRedisDel = async (...keys) => {
  try {
    ...
    console.error(`Auth Service: Redis DEL error for ${keys}:`, err.message);
    return 0;
  }
};
